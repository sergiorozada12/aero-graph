...
def test_model(df_train, df_test):
    ...
    return metrics, metrics_bal, metrics_0s
...
for feat in FEATS:
    ...
    for col in COLS:
        ...
        for i, entity in enumerate(entities):
            ...
            metrics = np.zeros(ITERS)
            metrics_bal = np.zeros(ITERS)
            metrics_0s = np.zeros(ITERS)

            procs = []
            with Pool(processes=N_CPUS) as pool:
                for i in range(ITERS):
                    procs.append(pool.apply_async(test_model, args=[df_train, df_test]))

                for i in range(ITERS):
                    metrics[i], metrics_bal[i], metrics_0s[i] = procs[i].get()
